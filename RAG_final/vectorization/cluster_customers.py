...
# ---------------- CONFIG ----------------
CHROMA_DIR       = "./chroma"
RESP_TEXT_COLL   = "excel_respondents_text"  # must match vector.py
N_CLUSTERS       = 6                         # you can change this if you want
...
def load_respondent_embeddings():
    ...
    try:
        ...
        raise SystemExit(
            f"Could not find collection '{RESP_TEXT_COLL}' in Chroma. "
            f"Error: {e}\nMake sure vector.py ran successfully."
        )

    print(f"[INFO] Fetching embeddings from collection '{RESP_TEXT_COLL}'...")
    # 'ids' are always returned even if not requested
    ...
    if len(ids) == 0 or len(embs) == 0:
        raise SystemExit(
            f"No embeddings found in collection '{RESP_TEXT_COLL}'. "
...
def train_kmeans(ids, X, metadatas):
    ...
    Returns:
        df_clusters: DataFrame with id, cluster, source
    ...
    kmeans = KMeans(
        ...
        n_init=10  # more compatible than 'auto' on older sklearn versions
    )
...
def main():
    ...
    # 1) Load embeddings from Chroma
    ids, X, metas = load_respondent_embeddings()

    # 2) Train KMeans and compute silhouette
    ...
    # 4) Save PCA-based visualization
